...
// Only the tent's top/back is solid — the whole sleeping-bag mat below it is walkable.
const shelterProp = { sortY: BLANKET.y+30, solid:true, box:[BLANKET.x+2, BLANKET.y, 20, 7], draw:()=>drawShelter(BLANKET.x, BLANKET.y) };
const props = [
...
function drawCharSprite(c, t) {
  ...
  if (charizard) {
    ...
    // keep the whole chest/torso orange (no cream belly) so the onesie reads solid
    ctx.fillStyle = '#e8862f'; ctx.fillRect(x+3, y+8, 9, 7);
    ctx.fillStyle = '#c46a1f'; ctx.fillRect(x+3, y+13, 9, 2);   // a little lower-belly shading, still orange
    // hood seam under the head + cream Charizard horns on the hood
...
function interact() {
  ...
  if (near(fx, fy, other.x+7, other.y+10, 16)) {        // a back-and-forth you click through
    ...
    fireLit = !fireLit;
    if (fireLit) L(night ? "You get the fire going. way less creepy out here with this lit." : "You get the fire going.", night ? "you get the fire going. okay, keep that on. please." : "you get the fire going. there it is.");
    else L("You kick dirt over the fire till it dies down.", night ? "you put the fire out. ...why would you do that right now." : "you put the fire out. too hot for it anyway.");
    ...
    L(pick(["You barely bump the card tent and it folds. we taped like 500 cards together and it can't take a breeze.", "You lean on the card tent and the whole thing caves in. so much for that."]),
      pick(["you bump the cards and the whole thing folds... there goes a Gengar.", "you lean on it and it caves. that was a $200 card holding up the roof."]));
    ...
    L(pick(["a whole pile of empty Coors out here. half of em are still full though. found beer.", "who drank all these. oh wait. it was us.", "somebody had a real night out here, huh."]),
      pick(["free Coors. not gonna question it.", "that's a lot of cans for two guys.", "we did not drink all those. ...did we?"]));
    ...
    L(pick(["this little table. honestly the comfiest, most normal thing we brought.", "set my drink down like an actual human for once."]),
      pick(["I love this dumb little table.", "tiny table. kinda perfect though."]));
    ...
    L(pick(["the Pokemon cooler. beers on the bottom, sausages up top. still cold too. I'd use this for real.", "popped the cooler open. this thing actually rips, not gonna lie."]),
      pick(["that cooler's sick.", "anything good left in there?"]));
  } else if (near(fx, fy, STOOL_Y.x+5, STOOL_Y.y+4, 8)) {
    L("the Pikachu stool. tiny little thing, but somehow the best seat we got.", "the Pikachu stool. that one's mine.");
  } else if (near(fx, fy, STOOL_B.x+5, STOOL_B.y+4, 8)) {
    L("Squirtle stool. my whole butt does not fit on this.", "Squirtle stool. solid pick.");
  } else if (near(fx, fy, STOOL_G.x+5, STOOL_G.y+4, 8)) {
    L("Bulbasaur stool. sits about two inches off the ground.", "Bulbasaur stool. only right answer for a starter.");
  } else if (near(fx, fy, STOOL_O.x+5, STOOL_O.y+4, 8)) {
    L("Charmander stool. matches the fire, I guess.", "Charmander stool. fire starter. get it.");
    ...
    L(pick(["my crocs. love em, but I hate how much sand gets stuck in all the little holes.", "crocs. instant sand collection the second I put em on."]),
      pick(["those are Luke's. little sand buckets.", "man will not take those off, ever."]));
    ...
    L(pick(["what's this old stump even doing out here? been dead a long time.", "somebody cut this stump down ages ago. wonder who camped here before us."]),
      pick(["that stump's older than both of us.", "sit on it? nah. splinters."]));
  } else if (near(fx, fy, DRIFT.x+10, DRIFT.y+4, 13)) {
    L(pick(["driftwood. river washed this up on the bar at some point. makes a decent bench.", "old piece of driftwood, all bleached out. been sitting here forever."]),
      pick(["good log to sit on. river dragged it up here.", "wonder how far this thing floated before it landed."]));
  } else if (near(fx, fy, MILKWEED.x+7, MILKWEED.y+8, 11)) {
    L("milkweed. leave it, the monarchs need that stuff.", "don't pull that, that's the butterfly plant.");
  } else if (near(fx, fy, BLANKET.x+12, BLANKET.y+18, 16)) {
    const rightBag = fx >= BLANKET.x + 12;
    a.action = 'lay'; a.x = BLANKET.x + (rightBag ? 10 : 0); a.y = BLANKET.y + 14;
    L(night ? "You get in the quilt. no tent over us, every little sound is something now." : "You crawl under the quilt. cowboy camping it.",
      night ? "you get under the quilt. not sleeping tonight, I keep hearing stuff." : "you get under the quilt. wake me up later.");
  } else if (near(fx, fy, CHAIR.x+6, CHAIR.y+6, 13))  { a.action = 'sit'; a.x = CHAIR.x; a.y = CHAIR.y; L("You drop into the chair. okay, this one's actually comfy.", "you kick back in the chair. now this is camping."); }
  else if (near(fx, fy, CHAIR2.x+6, CHAIR2.y+6, 13)) { a.action = 'sit'; a.x = CHAIR2.x; a.y = CHAIR2.y; L("You sink into the chair. could honestly fall asleep right here.", "you sink into the chair. not getting up for a while."); }
    ...
    L(pick(["if I pull a Magikarp out of here, I'm keeping it.", "one of these casts I'll actually catch something. probably not today."]),
      pick(["never catch a thing, but I look good standing here.", "somebody caught a real fish out here once, I swear."])); }
  else if (tcode === 'w' || tcode === 'W') { L("dude, this water is freezing.", "bro this water is so cold."); }
  else if (tcode === 'T') {
    if (night) L(pick(["something's moving back in the trees. I'm not looking.", "the woods get so loud at night out here."]),
                 pick(["that snapping turtle, man. it's still out there.", "I'm not walking into those trees right now."]));
           ...
           pick(["lotta trees, man.", "real bushcrafty back here.", "dude, that snapping turtle's been stalking us all day."]));
  }
  else if (tcode === 's') L(pick(["soft sand out on the bar.", "sand's already in the cooler somehow."]), "sand's gettin in everything, dude.");
         ...
         night ? pick(["stay by the fire, bro.", "I keep hearing the river and thinking it's footsteps."]) : pick(["unreal out here, man.", "I needed this, honestly.", "best spot we've found, no question."]));
}
function drawDialog() {
  if (!dialog) return;
